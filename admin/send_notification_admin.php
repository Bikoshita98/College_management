...
require_once "include/db.php";
 ?>
                ...
                <div class="row" style="min-height:500px">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <div class="x_panel">
                            ...
                            <div class="x_content">
							<form name="form1" action="send_notification_admin.php" method="post" class="col-lg-7">
								<div>
								...
								<div>
									<br><h4><label>To Faculty	</label><h4>
									<table class="table table-bordered">
										<tr>
											<td>
											<select class="form-control" name="fal_username">
											<?php
											$res=mysqli_query($connection,"select * from faculty_reg where status='yes'");
											while($row=mysqli_fetch_array($res))
											{	?>
												<option value="<?php echo $row["username"]?>">
												<?php	echo $row["dept"].' '.$row["name"]. "(".$row["username"].")" ;?>
												</option>
											<?php }
											?>
											...
											<tr>
												...
												<td>
												<label>Message	</label>
												<textarea class="form-control" name="msg2">
											...
											<tr>
												<td>
												<input type="submit" name="submit2" class="btn btn-primary" value="Send Message">
												</td>
											</tr>
								...
								<div>
									<br><h4><label>To Librarian	</label><h4>
									<table class="table table-bordered">
										<tr>
											<td>
											<select class="form-control" name="lib_username">
											<?php
											$res=mysqli_query($connection,"select * from librarian_reg where status='yes'");
											while($row=mysqli_fetch_array($res))
											{	?>
												<option value="<?php echo $row["username"]?>">
											...
											<tr>
												...
												<td>
												<label>Message	</label>
												<textarea class="form-control" name="msg3">
											...
											<tr>
												<td>
												<input type="submit" name="submit3" class="btn btn-primary" value="Send Message">
												</td>
											</tr>
		...
		if(isset($_POST["submit2"]))
		{
			...
			$DateTime;
			$query="INSERT INTO `messages`(`id`, `designation`, `susername`, `dusername`, `title`, `msg`, `datetime`, `read1`) 
			VALUES ('','faculty','$_SESSION[Username]','$_POST[fal_username]','$_POST[title2]','$_POST[msg2]','$DateTime','no')";
			$res=mysqli_query($connection,$query);
		
				if($res)
				{ ?>
				<script type="text/javascript">
					alert("Message send successfully to faculty");
					window.location="send_notification_admin.php";
		...
		if(isset($_POST["submit3"]))
		{
			...
			$DateTime;
			$query="INSERT INTO `messages`(`id`, `designation`, `susername`, `dusername`, `title`, `msg`, `datetime`, `read1`) 
			VALUES ('','librarian','$_SESSION[Username]','$_POST[lib_username]','$_POST[title3]','$_POST[msg3]','$DateTime','no')";
			$res=mysqli_query($connection,$query);
		
				if($res)
				{ ?>
				<script type="text/javascript">
					alert("Message send successfully to librarian");
					window.location="send_notification_admin.php";
